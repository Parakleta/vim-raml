...
syn region rootNamedParameterGroup matchgroup=field fold skip="^\s*\_[#]"
    \   start="^\z(\s*\)baseUriParameters:" end="^\(\z1\s\)\@!"
    \   contains=@yamlFlow,voidParameterGroup
syn region namedParameterGroup matchgroup=field fold contained skip="^\s*\_[#]"
    \   start="^\z(\s*\)uriParameters:" end="^\(\z1\s\)\@!"
    \   contains=@yamlFlow,voidParameterGroup
syn region voidParameterGroup matchgroup=parameter fold contained skip="^\s*\_[#]"
    \   start="^\z(\s*\)[^:]\+:" end="^\(\z1\s\)\@!"
...
syn region protocolGroup matchgroup=field fold skip="^\s*\_[#]"
    \   start="^\z(\s*\)protocols:" end="^\(\z1\s\)\@!"
    \   contains=@yamlFlow,protocol
syn keyword protocol contained HTTP HTTPS

" XXX This should require a YAML Collection
syn region rootDocumentationGroup matchgroup=field fold skip="^\s*\_[#]"
...
syn region securitySchemesGroup matchgroup=field fold skip="^\s*\_[#]"
...
syn region methodGroup matchgroup=method fold contained skip="^\s*\_[#]"
...
syn region methodFieldGroup matchgroup=methodField fold contained skip="^\s*\_[#]"
...
syn region methodBodyGroup matchgroup=methodBody fold contained skip="^\s*\_[#]"
...
syn region responseGroup matchgroup=method fold contained skip="^\s*\_[#]"
    \   start="^\z(\s*\)\d\{3\}:" end="^\(\z1\s\)\@!"
    \   contains=@yamlFlow,responseField,responseFieldGroup
syn keyword responseField contained description
syn region responseFieldGroup matchgroup=responseField fold contained skip="^\s*\_[#]"
